...
class DQN(nn.Module):
    def __init__(self, n_observations, n_actions):
        ...
        self.layer1 = nn.Linear(n_observations, 128)  # 建立 3 层网络
...
def select_action(state):
    ...
    )  # steps_done -> infty, eps_threshold -> eps_end
    steps_done += 1
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            )  # .view wraps value in a tensor container
    else:
        return torch.tensor(
...
def optimize_model():
    if len(memory) < BATCH_SIZE:
        return
    transitions = memory.sample(BATCH_SIZE)  # 在memory中随机抽取BATCH_SIZE个元素  [(state, action, next_state, reward), ...]
...
for i_episode in range(num_episodes):
    ...
    for t in count():  # 无限循环，count() 生成从 0 开始的无限数列
        action = select_action(state)
        observation, reward, terminated, truncated, _ = env.step(action.item())
        reward = torch.tensor([reward], device=device)  # wrap reward in tensor
